...
class Model extends Component implements IteratorAggregate, ArrayAccess, Arrayable
{
     ...
     * @event ModelEvent在[[validate(]]]开始时所提出的事件。你可以设置
     * [[ModelEvent::isValid]] to be false to stop the validation.
     * [[ModelEvent:isValid]]为false停止验证。
     */
    const EVENT_BEFORE_VALIDATE = 'beforeValidate';
    /**
     * @event Event an event raised at the end of [[validate()]]
     * @ Event事件在[[validate(]])结束时所提出的事件]
     */
     ...
     * 默认情况下，活动属性被认为是安全的，并且可以被大量分配。
     * If an attribute should NOT be massively assigned (thus considered unsafe),
     * 如果属性不应被大量分配(因此被认为是不安全的)，
     * please prefix the attribute with an exclamation character (e.g. `'!rank'`).
     *请用一个感叹号(例如:”!“)排名。
     * The default implementation of this method will return all scenarios found in the [[rules()]]
     * 此方法的默认实现将返回[[rules(]]]中所发现的所有场景]声明。
     * declaration. A special scenario named [[SCENARIO_DEFAULT]] will contain all attributes
     *  名为[[场景-默认]]的特殊场景将包含所有属性
     * found in the [[rules()]]. Each scenario will be associated with the attributes that
     * are being validated by the validation rules that apply to the scenario.
     *声明。一个名为[[场景-默认]]的特殊场景将包含在[规则()]中找到的所有属性。每个场景都将与应用于该场景的验证规则验证的属性相关联。
